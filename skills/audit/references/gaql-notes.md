...
| `shopping_performance_view` | (product perf) | **Works** — full product data flows through the Merchant LINK: `segments.product_item_id`, `product_title`, `product_brand`, `product_type_l1..5`, `product_condition`, `product_channel_exclusivity`, `product_custom_attribute0..4` + metrics, scoped to active campaigns. This is the D14 "which product burns / converts" pull (no Merchant connector needed — the products ARE in Google Ads). **Scope note:** it shows SERVING products only. The Merchant **approval STATUS layer** (disapproved / GTIN / policy) is NOT a GAQL field — that needs the Merchant Content API. **OOS, though, is reachable from the STORE connector** (Shopify/Woo inventory) — cross-ref `shopping_performance_view` item_ids against store stock to flag out-of-stock spenders WITHOUT a Merchant connector. So D14 degrades cleanly: performance (Ads) + OOS (store) always available; only disapproval/GTIN needs Merchant. |
